...
def train_realmlp_model(
    ...
    """
    Trains a RealMLP model using Cross Validation with Seed Ensembling via pytabkit.
    """
    ...
    print(f"--- Training RealMLP Model ({task}) with {len(random_states)} seeds ---")
    ...
    for seed in random_states:
        ...
        for fold, (train_idx, val_idx) in enumerate(kf.split(X, y)):
            ...
            if task == 'classification':
                model = RealMLP_TD_Classifier(random_state=seed, **params)
            else:
                model = RealMLP_TD_Regressor(random_state=seed, **params)
